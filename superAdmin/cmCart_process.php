...
if ($customerId && $userId && $paymentId) {
    ...
    try {
        ...
        if ($result->num_rows > 0) {
            ...
            $productIds = array_column($cartItems, 'product_id'); // Extract product IDs
            $placeholders = implode(',', array_fill(0, count($productIds), '?')); // Create placeholders

            $inventoryQuery = "SELECT product_id, stock FROM inventory WHERE product_id IN ($placeholders)";
            $stmt = $mysqli->prepare($inventoryQuery);
            $typeStr = str_repeat('i', count($productIds)); // Create type string for integers
            $stmt->bind_param($typeStr, ...$productIds);
            $stmt->execute();
            ...
            foreach ($cartItems as $item) {
                ...
                $updateQuery = "UPDATE inventory SET stock = ?, last_updated = NOW() WHERE product_id = ?";
